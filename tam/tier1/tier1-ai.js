...
// Supports passing command line arguments: node tam/tier1-ai.js <startIndex> <endIndex>
// Example: node tam/tier1-ai.js 0 25
const START_INDEX = parseInt(process.argv[2] ?? 0, 10);
const END_INDEX = parseInt(process.argv[3] ?? 200, 10);
...
const INPUT_FILE = path.join(__dirname, "tier1.json");
const PROMPT_FILE = path.join(__dirname, "tier1.md");
...
async function main() {
  ...
  // Process each person
  let scoredCount = 0;
  const counts = { "Tier 1": 0, "Maybe": 0, "Industry Partner": 0, "DNC": 0, "error/other": 0 };
  ...
  console.log("\n  Merging results safely onto latest snapshot of tier1.json...");
  ...
  fs.writeFileSync(INPUT_FILE, JSON.stringify(latestRaw, null, 2), "utf-8");

  console.log("\n" + "=".repeat(60));
  console.log(`  Done! ${scoredCount} people scored.`);
  console.log(`  Tier 1: ${counts["Tier 1"]}  Maybe: ${counts["Maybe"]}  Industry Partner: ${counts["Industry Partner"]}  DNC: ${counts["DNC"]}  Errors/Other: ${counts["error/other"]}`);
